"""
Arize Project Integration for Agent Evaluations
Creates a dedicated project with proper columns for agent evaluations
"""
...
class ArizeAgentEvaluationsProject:
    """
    Arize project integration for agent evaluations with dedicated columns
    """
    ...
    def __init__(self):
        ...
        self.project_name = "agent-evaluations"  # Dedicated project name
        ...
        if ARIZE_AVAILABLE and self.space_id and self.api_key:
            ...
            logger.warning("Arize Agent Evaluations Project not enabled. Missing credentials or dependencies.")
    ...
    def _initialize_arize(self):
        ...
        try:
            ...
            self.enabled = True
            logger.info(f"Arize Agent Evaluations Project initialized: {self.project_name}")
        except Exception as e:
            logger.error(f"Failed to initialize Arize Agent Evaluations Project: {str(e)}")
            self.enabled = False
    ...
    def log_query_evaluation(self, 
        ...
        """
        Log a complete query evaluation to the dedicated agent evaluations project
        """
        if not self.enabled:
            logger.warning("Arize Agent Evaluations Project not enabled, skipping query logging")
            return
        
        try:
            session_id = f"{user_id}_{int(time.time())}"
            
            # Prepare features with all the columns you requested
            features = {
                # Core columns
                "question": question,
                "response": response,
                ...
                "overall_query_score": 0.0  # Will be calculated
            }
            ...
            for agent_name, agent_data in agents_used.items():
                ...
                if anthropic_client:
                    try:
                        ...
                        # Add agent-specific columns as requested
            ...
            # Log to Arize as a dedicated project (not a model)
            self.client.log(
                model_id="query_evaluations",  # This creates the project structure
                model_version="1.0",
            ...
            logger.info(f"Logged query evaluation to {self.project_name} project for user {user_id}: Overall Score = {overall_query_score:.2f}")
            
        except Exception as e:
            logger.error(f"Failed to log query evaluation to Arize project: {str(e)}")
...
# Global instance
arize_agent_evaluations = ArizeAgentEvaluationsProject()
...
def log_query_evaluation(user_id: str, question: str, response: str, 
    ...
    """Convenience function to log query evaluation to dedicated project"""
    arize_agent_evaluations.log_query_evaluation(user_id, question, response, agents_used, anthropic_client)
